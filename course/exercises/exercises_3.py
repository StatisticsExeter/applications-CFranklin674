...
def bar_chart_means(df, cat_var, continuous_var, labels):
    ...
    group = df.groupby(cat_var)[continuous_var].mean().reset_index()
    fig = px.bar(group, x=cat_var, y=continuous_var, labels=labels)
    return fig
...
def stacked_bar_counts(df, cat_var_1, cat_var_2, labels):
    ...
    which summarises the mean of the continuous variable by different levels
    of cat_var and labels the axes using the labels dict."""
    group = df.groupby([cat_var_1, cat_var_2]).size().reset_index(name="count")
    fig = px.bar(group, x=cat_var_1, y="count", color=cat_var_2, labels=labels)
    return fig
